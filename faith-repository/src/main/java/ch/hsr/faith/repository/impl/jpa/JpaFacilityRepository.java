...
public interface JpaFacilityRepository extends JpaRepository<Facility, Long>, JpaSpecificationExecutor<Facility> {
	...
	@Query(value = "SELECT DISTINCT f from Facility f, PieceOfFurniture p, ItemNeeded i where f.id = i.facility.id and p.id = i.pieceOfFurniture.id and p.id = :pieceOfFurnitureId")
